...
- **fields**: Array of strings selecting which fields are shown. This is a membership test only — it does **not** reorder output; display order is fixed. Available fields are:
  ...
  - `dns`: Configured DNS nameservers. Read from `/etc/resolv.conf` on Linux and macOS, and from `GetAdaptersAddresses` on Windows — natively, with no PowerShell and no elevation. IPv4 nameservers only on Windows.
