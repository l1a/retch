...
# Rust release builds carry no debuginfo (there is no [profile.release] debug = true in
# Cargo.toml), so rpm's debuginfo extraction finds nothing and fails on an empty package.
...
# debug_package %{nil} above also disables rpm's automatic strip pass, so the
# binary ships unstripped unless we do it here (rpmlint: unstripped-binary-or-object).
strip %{buildroot}%{_bindir}/retch
...
target/release/retch --completions bash > %{buildroot}%{_datadir}/bash-completion/completions/retch
target/release/retch --completions zsh  > %{buildroot}%{_datadir}/zsh/site-functions/_retch
target/release/retch --completions fish > %{buildroot}%{_datadir}/fish/vendor_completions.d/retch.fish
...
%{_datadir}/bash-completion/completions/retch
%{_datadir}/zsh/site-functions/_retch
%{_datadir}/fish/vendor_completions.d/retch.fish
